...
public abstract class Task extends Activity implements ExternalViewChangeListener {
  ...
  public void run(String resourceName, int numPartitions, int partitionNum)
      throws Exception {
    if (!areParentTasksDone(getExternalViews())) {
      if (!parentDependencyLatch.await(TIMEOUT_SECONDS, TimeUnit.SECONDS)) {
        ...
        throw new Exception(getId()
            + " timed out while waiting for the following parent task to finish : " + pendingTasks);
      }
    }

    onEntry();
    runImpl(resourceName, numPartitions, partitionNum);
  }
  ...
  protected abstract void runImpl(String resourceName, int numPartitions, int partitionNum)
      throws Exception;
